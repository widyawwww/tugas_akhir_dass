<!-- Modal Edit Jadwan Konsultasi -->
<div id="edit-{{ $data->id }}" tabindex="-1"
    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        <div class="relative bg-white rounded-lg shadow-sm">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                <h3 class="text-xl font-semibold text-gray-900">Edit Jadwal Konsultasi</h3>
                <button type="button"
            ...
            <form method="POST" action="{{ route('admin.jadwal-psikolog.update', $data->id) }}" class="p-4 space-y-4">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <select name="status" id="status" class="w-full border rounded-lg px-3 py-2">
                        ...
                        <option value="confirmed" {{ $data->status == 'confirmed' ? 'selected' : '' }}>Confirmed
                        </option>
