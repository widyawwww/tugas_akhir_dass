        ...
        @forelse ($tips as $tips)
            <div class="bg-white shadow-md rounded-lg p-4 flex flex-col justify-between">
                <div>
                    ...
                    @if ($tips->gambar && file_exists(public_path('storage/' . $tips->gambar)))
                        <img src="{{ asset('storage/' . $tips->gambar) }}" alt="{{ $tips->nama }}"
                             class="w-full h-32 object-cover rounded mb-3">
                    @else
                        <p class="text-gray-400 italic">Tidak ada gambar atau file tidak ditemukan</p>
                    @endif
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button type="button"
                            class="bg-yellow-500 py-1 px-2 rounded-lg text-white text-sm"
            ...
            {{-- Modal Edit --}}
            <div id="edit-jenis-{{ $tips->id }}" tabindex="-1" class="hidden fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50">
                    ...
                    <div class="bg-white rounded-lg shadow p-4">
                        <form action="{{ route('admin.tips.update', $tips->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nama" value="{{ $tips->nama }}" class="w-full border p-2 rounded" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                @if ($tips->gambar)
                                    ...
                                    <img src="{{ asset('storage/' . $tips->gambar) }}" alt="Gambar" class="w-20 h-20 object-cover rounded mt-2">
                                @endif
                            </div>
        ...
        <div class="relative p-4 w-full max-w-md mx-auto mt-20">
            <div class="bg-white rounded-lg shadow p-4">
                <form action="{{ route('admin.tips.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label class="block mb-1 text-sm font-medium">Nama</label>
                        <input type="text" name="nama" class="w-full border p-2 rounded" placeholder="Contoh: Stres" required>
                    </div>
                    <div class="mb-3">
                        <label class="block mb-1 text-sm font-medium">Deskripsi</label>
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" class="bg-gray-400 text-white px-4 py-2 rounded" data-modal-hide="modalTambahJenis">Batal</button>
                        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Simpan</button>
                    </div>
                </form>
            </div>
